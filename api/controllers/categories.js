...
module.exports.new = async (req, res) => {
    ...
    if (!category_name) {
        res.status(422).json({ "mensagem": "Camp Name is mandatory!!" });
        return;
    }
    ...
    try {
        ...
        if (existingCategory.length >= 1) {
            return res.status(422).json({ "mensagem": "This category exisitng" });
        }
        await Categories.insertCategory(category_name);
        return res.status(200).json({ "mensagem": "Category insert with success!" });
...
module.exports.delete = async (req, res) => {
    ...
    try {
        ...
        if (existingId.length >= 1) {
            ...
            return res.status(200).json({ "mensagem": "Category delete with success" });

        }
        return res.status(422).json({ "mensagem": "Dont existing category with this id" });
...
module.exports.updateCategory = async (req, res) => {
    ...
    try {
        ...
        if (existingId.length >= 1) {
            ...
            return res.status(200).json({ "mensagem": "Category update with success" });
        }
        return res.status(422).json({ "mensagem": "Dont existing category with this id " });
